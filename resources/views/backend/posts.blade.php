  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      {{csrf_field()}}
        <div class="form-group">
          ...
          <input required type="text" name='mabaiviet' class="form-control" id="exampleInputName"  aria-describedby="nameHelp">
    ...
    <div class="table-responsive">
      <table class="table table-striped b-t b-light">
        ...
        <tbody>
        @foreach($poslist as $posts)
          <tr>
          
            <td><label class="i-checks m-b-none"><input type="checkbox" name="post[]"><i></i></label></td>
            <td>{{$posts->pos_Mabaiviet}}</td>
            <td>{{$posts->pos_Tieudebaiviet}}</td>
            <td>{{$posts->pos_Noidungbaiviet}}</td>
            <td>
              <img width="200px" src="{{asset('storage/app/avatar/'.$posts->pos_Hinhanh)}}" class="thumbnail">
            </td>
            <td>{{$posts->pos_Ngaytao}}</td>
           
            <td><span class="text-ellipsis"></span></td>
            <td>
            <a href="{{asset('admin/posts/edit/'.$posts->pos_id)}}"  class="btn btn-success" > <i class="fas fa-tools"></i>
  
</a>
         
  

      <a onclick="return confirm('Bạn có muốn xóa không!')" href="{{asset('admin/posts/delete/'.$posts->pos_id)}}" type="button" class="btn btn-danger"  data-target="#exampleModal">
